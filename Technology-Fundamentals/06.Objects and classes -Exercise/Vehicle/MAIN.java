...
public class MAIN {
    public static void main(String[] args) throws IOException {
        ...
        String input = "";
        List<Vehiclesort> vehicles = new ArrayList<>();
        while (!"End".equals(input = reader.readLine())) {
            String[] data = input.split("\\s+");
            Vehiclesort vehicle = new Vehiclesort(data[0], data[1], data[2], Integer.parseInt(data[3]));
    ...
    private static double average(List<Vehiclesort> vehicles) {
        if (vehicles.size()==0){
            return 0.0;
        }
        double sum = 0;
        for (Vehiclesort vehicle : vehicles) {
